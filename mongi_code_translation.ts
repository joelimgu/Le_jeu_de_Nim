...
type StartingEntity = "player" | "computer"
 ...
 */
function sumXor(list: number[]): number {
...
function pairPosition(list: number[]): boolean {
...
function initList() {
    let res: string | null = "0"
    const list: number[] = []
    while ( res ) {
        res = prompt(`Donner le nombre d'allumettes de la ligne n°${list.length} :`)
        if ( res && parseInt(res) ) list.push(parseInt(res));
    }
    return list
}

function affichage(list: number[]) {
    list.forEach( (el) => {
        let line = ""
        for ( let k = 0; k < el; k++ ){
            line += " |"
...
function playRandomly(list: number[]) {
...
function playStrategy(list: number[]) {
    let found = false
    let l = 0
    let line: number | undefined = undefined
    let nbToRemove: number | undefined = undefined
...
// noinspection JSUnusedGlobalSymbols
function playStrategyMax(list: number[]) {
...
function computerTurn(list: number[]) {
...
function playerTurn(list: number[]) {
    console.log("player's turn")
    let correct = false
    let nbToRemove: number = 0
    let lineNb: number | null = null
    while ( !correct ) {
        const lineStr: string | null = prompt(`Choisir une ligne entre 0 et ${list.length - 1} :`)
...
function jeuDeNim() {
    ...
    affichage(gameList)
    let correct = false
    let start: StartingEntity = "player"
    while ( !correct ) {
    ...
    if ( start === "player") {
        while ( !fin_du_jeu ) {
            playerTurn(gameList)
            if ( !fin_du_jeu ) computerTurn(gameList);
        }
    } else {
        while ( !fin_du_jeu ) {
            computerTurn(gameList)
            if ( !fin_du_jeu ) playerTurn(gameList);
        }
    }
